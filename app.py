...
# Function to plot monthly cashflow (Income vs Expense)
def plot_monthly_cashflow(df):
    ...
    monthly.plot(kind='bar', stacked=False, ax=ax, color=['#4CAF50', '#FF7043']) # Added colors
    plt.title('Fluxo de Caixa Mensal: Entradas vs. Saídas', fontsize=14) # Adjusted font size
    plt.ylabel('Valor (R$)', fontsize=10) # Adjusted font size
    plt.xlabel('Mês', fontsize=10) # Adjusted font size
...
def plot_cumulative_balance(df):
    ...
    plt.title('Saldo Líquido Acumulado ao Longo do Tempo', fontsize=14) # Adjusted font size
    plt.ylabel('Saldo (R$)', fontsize=10) # Adjusted font size
    plt.xlabel('Data', fontsize=10) # Adjusted font size
    ax.yaxis.set_major_formatter(mticker.FormatStrFormatter('R$%.2f'))
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    return fig

# Function to plot expense distribution by category
def plot_expense_distribution(df):
    ...
    plt.title('Distribuição de Despesas por Categoria (Total)', fontsize=14) # Adjusted font size
    plt.xlabel('Valor (R$)', fontsize=10) # Adjusted font size
    plt.ylabel('Categoria', fontsize=10) # Adjusted font size
...
def plot_monthly_category_expenses(df):
    ...
    plt.title('Evolução Mensal das Despesas por Categoria', fontsize=14) # Adjusted font size
    plt.ylabel('Valor (R$)', fontsize=10) # Adjusted font size
    plt.xlabel('Mês', fontsize=10) # Adjusted font size
    plt.xticks(rotation=45, ha='right', fontsize=8) # Adjusted font size
    ax.yaxis.set_major_formatter(mticker.FormatStrFormatter('R$%.2f'))
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.legend(title='Categoria', bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=8) # Adjusted font size
    plt.tight_layout()
    return fig
...
def plot_monthly_income(df):
    ...
    plt.title('Evolução Mensal da Receita', fontsize=14) # Adjusted font size
    plt.ylabel('Valor (R$)', fontsize=10) # Adjusted font size
    plt.xlabel('Mês', fontsize=10) # Adjusted font size
    ax.yaxis.set_major_formatter(mticker.FormatStrFormatter('R$%.2f'))
    plt.grid(True, linestyle='--', alpha=0.7)
...
def plot_percentage_recurring_expenses(df):
    ...
    plt.title('Percentual de Despesas Recorrentes Mensais', fontsize=14) # Adjusted font size
    plt.ylabel('Percentual (%)', fontsize=10) # Adjusted font size
    plt.xlabel('Mês', fontsize=10) # Adjusted font size
...
def main():
    ...
    # Fluxo de Caixa Mensal: Should be affected by both date and category filters (income and filtered expenses)
    cashflow_filtered_df = date_filtered_df.copy()
    if 'All' not in selected_categories:
        cashflow_filtered_df = cashflow_filtered_df[cashflow_filtered_df['Categoria'].isin(selected_categories) | (cashflow_filtered_df['Tipo'] == 'Income')]

    # Saldo Líquido Acumulado: Should be affected by both date and category filters (income and filtered expenses)
    cumulative_balance_filtered_df = date_filtered_df.copy()
    if 'All' not in selected_categories:
        cumulative_balance_filtered_df = cumulative_balance_filtered_df[cumulative_balance_filtered_df['Categoria'].isin(selected_categories) | (cumulative_balance_filtered_df['Tipo'] == 'Income')]
    ...
    income_filtered_df = date_filtered_df.copy()
    ...
    recurring_percentage_filtered_df = date_filtered_df.copy()
    if 'All' not in selected_categories:
        # Apply category filter only to expenses for the total expenses calculation
        recurring_percentage_filtered_df = recurring_percentage_filtered_df[recurring_percentage_filtered_df['Categoria'].isin(selected_categories) | (recurring_percentage_filtered_df['Tipo'] == 'Income')]
    ...
    monthly_net_balance = monthly_total_income.sub(monthly_total_expenses, fill_value=0)
    ...
    if filter_type == 'Mês Específico' and selected_months:
        ...
        if not monthly_total_expenses.empty and current_month in monthly_total_expenses.index:
             last_month_expenses_by_category = (cashflow_filtered_df[
                (cashflow_filtered_df['Tipo'] == 'Expense') &
                (cashflow_filtered_df['AnoMes'] == current_month)]
                .groupby('Categoria')['Valor']
                .sum()
                .sort_values(ascending=False)
            )
            ...
            st.write("Nenhuma despesa total mensal disponível para o período selecionado.")
        ...
        if not monthly_total_expenses.empty:
            last_month_anomes = monthly_total_expenses.index[-1]
            last_month_expenses_by_category = (cashflow_filtered_df[
                (cashflow_filtered_df['Tipo'] == 'Expense') &
                (cashflow_filtered_df['AnoMes'] == last_month_anomes)]
                .groupby('Categoria')['Valor']
                .sum()
                .sort_values(ascending=False)
            )
    ...
    st.markdown("Comparativo mensal entre entradas e saídas e a evolução da receita.")
    ...
    col3, col4 = st.columns(2) # Keeping two columns for potential future use or alignment

    with col3:
        st.subheader('Distribuição de Despesas por Categoria')
        fig3 = plot_expense_distribution(expense_distribution_filtered_df)
